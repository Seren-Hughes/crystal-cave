          ...
          <div class="text-container">
              ...
              <span class="instructions">${
                currentMessageIndex === speechBubbleMessages.length - 1 ?
                  (isTouchDevice ? "(Tap to start the game!)" : "(Click or press spacebar to start the game!)") :
                  (isTouchDevice ? "(Tap to continue...)" : "(Click or press spacebar to continue...)")
              }</span>
          </div>
      `;
}
...
function initializeGameSite() {
  ...
  const audioUserEventOverlay = document.querySelector(
    ".audio-user-event-overlay"
  );
  const speechBubble = document.querySelector(".speech-bubble");
  const overlay = document.querySelector(".overlay");
  ...
  audioUserEventOverlay.addEventListener("click", () => {
    ...
    allButtons.forEach((btn) =>
      btn.classList.remove("hidden-on-audio-overlay")
    );
  ...
  .addEventListener("click", () => {
    ...
    document.querySelectorAll(".volume-slider").forEach((slider) => {
      slider.addEventListener("input", (event) => {
        ...
        if (soundType === "ambient" && volume > 0)
          audioManager.isAmbientMuted = false;
        if (soundType === "background" && volume > 0)
          audioManager.isBackgroundMuted = false;
        if (soundType === "effects" && volume > 0)
          audioManager.isEffectsMuted = false;
    ...
    const bodyFilter =
      getComputedStyle(document.body).getPropertyValue("filter") || "";
    const brightnessPart = bodyFilter
      .trim()
      .split(/\s+/)
      .find((part) => part.startsWith("brightness("));
...
function clearAllTimeouts() {
  ...
  for (let color in crystalTimeouts) {
    if (Object.prototype.hasOwnProperty.call(crystalTimeouts, color)) {
...
function syncAudioSettingsUI() {
  document.querySelectorAll(".volume-slider").forEach((slider) => {
    ...
    const muteButton = document.querySelector(
      `.mute-toggle[data-sound="${soundType}"]`
    );
    if (muteButton) {
      if (isMuted) {
